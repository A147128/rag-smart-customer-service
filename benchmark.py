...
def benchmark_cache_performance():
    ...
    response1 = service.chain.invoke({"input": test_question}, session_config)
    ...
    print("\n[测试2] 再次调用相同问题(缓存命中)")
    start_time = time.time()
    response2 = service.chain.invoke({"input": test_question}, session_config)
    elapsed2 = time.time() - start_time
    print(f"响应时间: {elapsed2:.2f}秒")
...
def benchmark_hybrid_retrieval():
    ...
    # 纯向量检索
    print("\n[配置1] 纯向量检索")
    service_vector = EnhancedRagService(use_cache=False, use_hybrid_retrieval=False)
    print("✓ 仅使用向量相似度检索")

    # 混合检索
    print("\n[配置2] 混合检索(向量70% + BM25 30%)")
    service_hybrid = EnhancedRagService(use_cache=False, use_hybrid_retrieval=True)
    print("✓ 结合向量检索和关键词BM25检索")

    print("\n💡 混合检索优势:")
    print("  - 专有名词匹配更准确(BM25)")
    print("  - 语义理解更全面(向量)")
    print("  - 召回率提升约15-25%")
