...
class ResponseCache:
    ...
    def __init__(self, cache_file="./response_cache.json", ttl_hours=24) -> None:
    ...
    def _save_cache(self) -> None:
        """保存缓存到文件"""
        # os.makedirs(文件夹路径,exist_ok)专门用来创建文件夹，存在就没事，不存在就给你创建
    ...
    def get(self, question: str) -> str | None:
        ...
        if key in self.cache:
            ...
            if datetime.now() - cache_time < self.ttl:
                logger.info(f"[缓存命中] 问题: {question[:30]}...")
    ...
    def set(self, question: str, response: str) -> None:
        """设置缓存"""
        key = self._generate_key(question)
        self.cache[key] = {"question": question, "response": response, "timestamp": datetime.now().isoformat()}
        self._save_cache()
        logger.info(f"[缓存写入] 问题: {question[:30]}...")
