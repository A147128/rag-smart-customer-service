...
_COLLECT_SYSTEM = (
    ...
    '3. 如果用户反馈或追问（如"不够正式"），场合已确定，设置complete=true。\n'
    "4. 每次只问 1 个问题，逐步收集场合、季节、风格、预算等信息。\n"
    "5. 场合+季节+风格+预算完整时，complete=true并填写occasion；否则complete=false\n"
    "6. 必须以JSON格式输出，不要包含其他内容。\n"
    "输出JSON：complete/Bool, occasion/场合, missing/[缺少], question/问题"
)
...
def generate_outfit(state: dict) -> dict:
    ...
    llm = ChatTongyi(model=config.chat_model_name)
    # Use stream() for token-level generation, collecting all chunks
    chunks = []
    ...
    outfit_plan = "".join(chunks) or "无法生成推荐，请稍后再试"
